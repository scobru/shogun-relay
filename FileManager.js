...
class FileManager {
  ...
  configureMulter() {
    console.log(`Configuring Multer - IPFS ${this.config.ipfsManager?.isEnabled() ? "enabled" : "disabled"}`);
  ...
  async getFileById(fileId) {
    ...
    await new Promise((resolve) => {
      const timeout = setTimeout(() => {
        console.log(`Timeout reached after 3 seconds for file ${fileId}`);
        resolve();
      }, 3000);
  ...
  async deleteFile(fileId) {
    ...
    await new Promise((resolve) => {
      ...
      // Timeout in case Gun doesn't respond
  ...
  async saveFileMetadata(metadata) {
    ...
    await new Promise((resolve, reject) => {
      this.config.gun
        ...
        .put(metadata, (ack) => {
          if (ack.err) {
            console.error("[FILE-MANAGER] Error saving to GunDB:", ack.err);
            reject(new Error("Error saving to GunDB: " + ack.err));
          } else {
            console.log("[FILE-MANAGER] File saved successfully to GunDB");
            resolve();
          }
        });
      ...
      await new Promise((resolve) => {
        this.config.gun
          ...
          .once((data) => {
            if (data) {
              savedData = data;
              console.log("[FILE-MANAGER] Save verification: file found in GunDB");
            } else {
              console.warn("[FILE-MANAGER] Save verification: file not found in GunDB");
            }
            resolve();
          });
