...
ARG DISABLE_RADISK
ARG GUN_PEERS
# Multi-chain configuration (passed by CapRover)
ARG X402_NETWORKS
ARG X402_DEFAULT_NETWORK
ARG X402_BASE_SEPOLIA_RPC
ARG X402_BASE_RPC
ARG BRIDGE_NETWORKS
ARG BRIDGE_DEFAULT_NETWORK
ARG BRIDGE_BASE_SEPOLIA_RPC
ARG BRIDGE_BASE_RPC
ARG DEALS_NETWORKS
...
ARG ANNAS_ARCHIVE_TORRENT_LIST_URL
ARG ANNAS_ARCHIVE_DATA_DIR
ARG ANNAS_ARCHIVE_MAX_TB
...
# openssl-dev provides SSL headers for node-datachannel
RUN apk add  \
    git \
    curl \
    wget \
