...
export function setSignal(initialValue, options = {}) {
  const { key, element, bypass } = options || {};
  ...
  const write = (newValue) => {
    ...
    // If bypass is true, skip GunDB storage (useful for arrays)
    if (resolvedKey && gun && !bypass) {
      // Don't attempt to store arrays directly in GunDB
      const isArray = Array.isArray(value);
      if (!isArray) {
        gun.get(resolvedKey).put({ value });
      } else {
        console.debug(`[setSignal] Skipping GunDB storage for array at ${resolvedKey}`);
      }
    }
