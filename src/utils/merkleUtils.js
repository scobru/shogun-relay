...
export class MerkleManager {
  ...
  async initialize() {
    try {
      ...
      console.log(`Merkle tree initialized with ${this.allowedPubKeys.length} keys`);
      return this.getRoot();
    } catch (error) {
  ...
  async extractPubKeysFromRadata() {
    ...
    try {
      console.log(`Searching for pub keys in: ${path.resolve(this.radataPath)}`);
      
      // List all files and directories in radata
      const entries = fs.readdirSync(this.radataPath);
      console.log(`Found ${entries.length} entries in radata directory:`, entries);
          ...
          try {
            ...
            try {
              ...
              if (keysFromJson.length > 0) {
                console.log(`Found ${keysFromJson.length} keys in file ${entry}`);
                keysFromJson.forEach(key => {
                  if (!pubKeys.some(existing => existing.equals(Buffer.from(key, 'hex')))) {
                    pubKeys.push(Buffer.from(key, 'hex'));
                    console.log(`Added pub key from ${entry}: ${key.substring(0, 10)}...`);
                  }
                });
              }
            } catch (jsonError) {
              console.log(`File ${entry} is not valid JSON, trying regex parsing`);
              ...
              while ((match = pubKeyRegex.exec(content)) !== null) {
                ...
                if (pubKey && pubKey.length > 20) { // Simple validation
                  if (!pubKeys.some(existing => existing.equals(Buffer.from(pubKey, 'hex')))) {
                    pubKeys.push(Buffer.from(pubKey, 'hex'));
                    console.log(`Added pub key via regex from ${entry}: ${pubKey.substring(0, 10)}...`);
                  }
                }
              }
            }
          } catch (fileError) {
            console.warn(`Error processing file ${entry}: ${fileError.message}`);
          }
