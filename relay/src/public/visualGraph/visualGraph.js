...
function update() {
  console.log(
    "Updating visualization with",
    window.graph?.nodes?.length || 0,
    "nodes and",
    window.graph?.edges?.length || 0,
    "edges"
  );
  ...
  node.append("title").text(function (d) {
    return d.id ? d.id.toUpperCase() : "Unknown";
  });
  ...
  console.log(
    "Updating simulation with",
    window.graph.nodes.length,
    "nodes and",
    window.graph.edges.length,
    "edges"
  );
  ...
  simulation.force("link").links(window.graph.edges);
...
function detail(d, i, nodes) {
  ...
  if (window.gun && key) {
    ...
    window.gun.get(key).once((data, nodeKey) => {
      ...
      var hasActualData =
        data && Object.keys(data).some((k) => k !== "_" && data[k] != null);
...
function displayNodeData(data, key) {
  ...
  console.log("displayNodeData called with:", {
    data,
    key,
  ...
  if (!data) {
    detailContainer.innerHTML =
      "<div class='contV'><h3>Data Inspector</h3><p>No data found for key: " +
      key +
      "</p></div>";
    return;
  }
  ...
  if (!hasActualData) {
    detailContainer.innerHTML =
      "<div class='contV'><h3>Data Inspector</h3><p>Empty data for key: " +
      key +
      "</p><pre>" +
...
function loadAllNodesMode() {
  ...
  if (typeof window.loadAllNodesDirectly === "function") {
    ...
    updateStatus(
      "❌ Load All Nodes function not available - please wait for modules to load",
      true
    );

    // Retry after a short delay in case the function is still loading
    setTimeout(() => {
      if (typeof window.loadAllNodesDirectly === "function") {
        console.log(
          "✅ loadAllNodesDirectly function now available, retrying..."
        );
        ...
        updateStatus("❌ Load All Nodes function failed to load", true);
      }
    }, 1000);
  }
}
