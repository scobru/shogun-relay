  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      if (
        ...
        repResult.value.success &&
        repResult.value.leaderboard
      ) {
        setLeaderboard(repResult.value.leaderboard);
      }
      if (
        peersResult.status === "fulfilled" &&
        peersResult.value.success &&
        Array.isArray(peersResult.value.peers)
      ) {
      ...
      if (
        relaysResult.status === "fulfilled" &&
        relaysResult.value.success &&
        Array.isArray(relaysResult.value.relays)
      ) {
      ...
      <div className="card bg-base-100 shadow">
        <div className="card-body">
          ...
          <div className="stats stats-vertical lg:stats-horizontal w-full">
            <div className="stat">
              <div className="stat-title">Relays</div>
              <div className="stat-value text-primary">{formatNumber(stats.totalRelays)}</div>
              <div className="stat-desc">{formatNumber(stats.activeRelays)} active</div>
            </div>
            <div className="stat">
              <div className="stat-title">Connections</div>
              <div className="stat-value">{formatNumber(stats.totalConnections)}</div>
            </div>
            <div className="stat">
              ...
              <div className="stat-desc">{storageGB >= 1 ? "GB" : "MB"}</div>
            </div>
            <div className="stat">
              <div className="stat-title">Pins</div>
              <div className="stat-value">{formatNumber(stats.totalPins)}</div>
            </div>
          </div>
        </div>
      </div>
