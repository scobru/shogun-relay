...
function Services() {
    ...
    try {
      ...
      if (data.success && data.services) {
        const serviceList: ServiceStatus[] = Object.entries(data.services).map(([key, value]: [string, any]) => {
          ...
          return { name: displayName, status: status as 'online' | 'offline', uptime: value.uptime, pid: value.pid || 0 }
        })
  ...
  const handleServiceAction = async (serviceName: string, action: 'restart' | 'stop' | 'start') => {
    ...
    try {
      ...
      setTimeout(() => { fetchServices(); setLoadingAction(null) }, 2000)
    } catch (e) {
      console.error(e)
  ...
  const handleGlobalAction = async (action: 'health' | 'restart-all') => {
    ...
    try {
      if (action === 'health') {
        ...
        await fetch('/api/v1/system/services/restart-all', { method: 'POST', headers: getAuthHeaders() })
        alert('Restart sent.')
      ...
      <div className="card bg-base-100 shadow">
        <div className="card-body flex-row items-center justify-between">
          ...
          <div className="flex gap-2">
            <button className="btn btn-outline btn-sm" onClick={() => { setRefreshing(true); fetchServices() }} disabled={refreshing}>
              {refreshing ? <span className="loading loading-spinner loading-xs"></span> : '🔄'} Refresh
            </button>
            <button className="btn btn-primary btn-sm" onClick={() => handleGlobalAction('health')} disabled={refreshing}>
              ❤️ Health
            </button>
            <button className="btn btn-warning btn-sm" onClick={() => handleGlobalAction('restart-all')} disabled={refreshing}>
              ⚡ Restart All
            </button>
          </div>
        </div>
            ...
            <div key={svc.name} className="card bg-base-100 shadow">
              <div className="card-body">
                <div className="flex items-center justify-between">
                  ...
                  <div className={`badge ${svc.status === 'online' ? 'badge-success' : 'badge-error'} gap-1`}>
                    <span className={`w-2 h-2 rounded-full ${svc.status === 'online' ? 'bg-success' : 'bg-error'}`}></span>
                    {svc.status}
                  </div>
                </div>
                ...
                <div className="card-actions justify-end mt-4">
                  <button className="btn btn-ghost btn-xs" onClick={() => fetchLogs(svc.name)}>📃 Logs</button>
                  <button className="btn btn-warning btn-xs" onClick={() => handleServiceAction(svc.name, 'restart')} disabled={loadingAction === svc.name}>
                    {loadingAction === svc.name ? <span className="loading loading-spinner loading-xs"></span> : '⚡'} Restart
                  </button>
                  {svc.status === 'online' ? (
                    <button className="btn btn-error btn-xs" onClick={() => handleServiceAction(svc.name, 'stop')} disabled={loadingAction === svc.name}>
                      🛑 Stop
                    </button>
                  ) : (
                    <button className="btn btn-success btn-xs" onClick={() => handleServiceAction(svc.name, 'start')} disabled={loadingAction === svc.name}>
                      ▶ Start
                    </button>
                  )}
                </div>
          ...
          <div className="modal-box max-w-3xl">
            ...
            <div className="bg-base-300 rounded-lg p-4 mt-4 max-h-96 overflow-y-auto font-mono text-xs">
              {serviceLogs.map((log, i) => (
                <div key={i} className="py-0.5 border-b border-base-content/10">{log}</div>
              ))}
            </div>
