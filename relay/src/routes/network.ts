    ...
    try {
      ...
      for (const entry of frozenEntries) {
        if (
          ...
          const deal = entry.data as unknown as StorageDeals.Deal;
      ...
      loggers.server.info(
        ...
        `   📋 Found ${allDeals.length} deals in frozen storage`
      );
      ...
      loggers.server.info(
        {
          activeDeals: stats.totalActiveDeals,
          totalSizeMB: stats.totalDealStorageMB,
          uniqueCids: activeDealCids.size,
        },
      ...
      if (errorStack) {
        loggers.server.error({ stack: errorStack }, `   Stack: ${errorStack}`);
      }
    }
    ...
    if (stats.totalStorageBytes === 0 && stats.totalPins === 0) {
      ...
      try {
        ...
        if (pinLs && typeof pinLs === "object" && "Keys" in pinLs) {
          ...
          if (keys) {
            stats.totalPins += Object.keys(keys).length;
            loggers.server.info(
              { pinCount: Object.keys(keys).length },
              `   ✅ IPFS pins: ${Object.keys(keys).length}`
            );
          }
        }
      } catch (ipfsError) {
