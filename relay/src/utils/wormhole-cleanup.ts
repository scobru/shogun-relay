...
async function runWormholeCleanup(gun: IGunInstance): Promise<void> {
    ...
    try {
        ...
        log.debug({ cutoffTime: new Date(cutoffTime).toISOString() }, "🔄 Scanning for orphaned wormhole transfers");
        ...
        for (const { code, data } of transfers) {
            try {
                ...
                gun.get("shogun-wormhole").get("wormhole-transfers").get(code).put(null as any);
 ...
 */
async function getWormholeTransfers(gun: IGunInstance): Promise<Array<{ code: string; data: WormholeTransfer }>> {
