  ...
  private async addFileToIPFS(filePath: string): Promise<string | null> {
    try {
      ...
      const curlArgs = [
        '-s',
        '-X', 'POST',
        `${ipfsApiUrl}/api/v0/add?pin=true&quiet=true`
      ];

      // Add Authorization header if token is configured
      if (ipfsApiToken) {
        curlArgs.push('-H', `Authorization: Bearer ${ipfsApiToken}`);
      }

      curlArgs.push('-F', `file=@${filePath}`);

      const debugArgs = curlArgs.map(arg => arg === `Authorization: Bearer ${ipfsApiToken}` ? 'Authorization: Bearer ***' : arg).join(' ');
      loggers.server.debug(`📚 Running IPFS add: curl ${debugArgs}`);
