...
export class TorrentManager {
  ...
  private async loadTorrents(): Promise<void> {
      ...
      if (await fs.promises.access(torrentsFile).then(() => true).catch(() => false)) {
          try {
              const fileContent = await fs.promises.readFile(torrentsFile, 'utf8');
           ...
           try {
               await fs.promises.writeFile(torrentsFile, JSON.stringify([], null, 2));
           } catch (error) {}
      }
   ...
   */
  private async persistTorrent(magnetOrPath: string, options?: { filePaths?: string[], infoHash?: string, name?: string, isSeeded?: boolean }): void {
      try {
          const torrentsFile = path.join(this.dataDir, 'torrents.json');
          ...
          if (!alreadyExists) {
              ...
              await fs.promises.writeFile(torrentsFile, JSON.stringify(existingTorrents, null, 2));
          }
      } catch (error) {
  ...
  public async refetchDynamicTorrents(maxTb?: number): Promise<{ added: number; skipped: number; total: number }> {
    ...
    for (const magnet of magnets) {
      ...
      await this.persistTorrent(magnet);
  ...
  public addTorrent(magnetOrPath: string): any {
    ...
    try {
      ...
      await this.persistTorrent(magnetOrPath);
        ...
        this.client!.seed(filePaths, {
          ...
          try {
            ...
            if (aacRecords.length > 0) {
              ...
              await fs.promises.writeFile(metadataFile, jsonlContent);
            ...
            await this.persistTorrent(torrent.magnetURI, {
   ...
   */
  private async loadCatalog(): void {
    try {
      if (await fs.promises.access(this.catalogFile).then(() => true).catch(() => false)) {
        const data = await fs.promises.readFile(this.catalogFile, 'utf8');
   ...
   */
  private async saveCatalog(): void {
    try {
      const entries = Array.from(this.catalog.values());
      await fs.promises.writeFile(this.catalogFile, JSON.stringify(entries, null, 2));
    ...
    try {
      ...
      if (await fs.promises.access(torrentsFile).then(() => true).catch(() => false)) {
        ...
        if (filtered.length !== existingTorrents.length) {
          await fs.promises.writeFile(torrentsFile, JSON.stringify(filtered, null, 2));
