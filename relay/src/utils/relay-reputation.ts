...
export function calculateReputationScore(metrics: ReputationMetrics): ReputationScore {
  ...
  // Based on storage proof reliability
  const totalProofs = metrics.proofsTotal || 0;
  const totalSuccessfulProofs = metrics.proofsSuccessful || 0;
  ...
  // Based on storage proof response time
  // Inverse scale: faster = higher score
  const avgResponseTime = metrics.avgResponseTimeMs;
...
export function initReputationTracking(gun: GunInstance, relayHost: string): GunNode {
  const reputationNode = getGunNode(gun, GUN_PATHS.REPUTATION).get(relayHost);
...
export async function recordProofSuccess(
  ...
  try {
    if (!observerKeyPair) {
      ...
      const node = getGunNode(gun, GUN_PATHS.REPUTATION).get(relayHost);
