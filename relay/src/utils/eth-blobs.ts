...
export async function fetchBlobData(txHash: string): Promise<BlobData> {
  ...
  try {
    ...
    loggers.server.warn(
      "⚠️ fetching from live RPC not fully verified in this environment, using mock data for safety."
    );
    return mockFetchBlobData(txHash);
  } catch (error: any) {
  ...
  try {
    ...
    try {
      ...
      if (!e.message.includes("already loaded")) {
        loggers.server.warn(
          "⚠️ Failed to load trusted setup, might be already loaded or missing: " + e.message
        );
      }
    }
