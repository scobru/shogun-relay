...
export async function startBridgeListener(
  ...
  try {
    ...
    // Start listening to Deposit events
    const cleanup = await client.listenToDeposits(config.startBlock || "latest", async (event) => {
      try {
        ...
        if (!receipt) {
          log.warn({ txHash: event.txHash }, "Transaction receipt not found, skipping");
          return;
        }
        ...
        if (verifyBalance < expectedBalance) {
          ...
          throw new Error(
            `Balance verification failed: expected ${expectedBalance.toString()}, got ${verifyBalance.toString()}`
          );
        }
        ...
        log.error({ error, event }, "Error processing deposit event");
    ...
    const forceCleanup = await client.listenToForceWithdrawals(
      ...
      async (event) => {
        try {
          ...
          log.debug(
            { withdrawalHash: event.withdrawalHash },
            "Force withdrawal added to pending queue"
          );
        } catch (error) {
