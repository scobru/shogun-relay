          ...
          await new Promise<void>((nextAlias) => {
            gun.get(aliasSoul).once(async (pubs: any) => {
              ...
              if (pubKeys.length > 1) {
                ...
                log.warn(
                  { alias: aliasName, count: pubKeys.length, pubKeys },
                  "Found duplicate alias registrations"
                );
                ...
                for (const pub of pubKeys) {
                  ...
                  if (existsInUsers) {
                    bestPub = pub;
                    log.info(
                      { alias: aliasName, bestPub },
                      "Found valid identity for alias in users index"
                    );
                    break;
                  }
                }
                ...
                if (!bestPub) {
                  ...
                  log.info(
                    { alias: aliasName, bestPub },
                    "No identity found in users index, keeping first found"
                  );
                }
                ...
                await Promise.all(
                  pubKeys.map(async (pub) => {
                    if (pub !== bestPub) {
                      log.info(
                        { alias: aliasName, pub },
                        "Removing redundant identity association"
                      );
                      return new Promise((resolve) => {
                        gun
                          .get(aliasSoul)
