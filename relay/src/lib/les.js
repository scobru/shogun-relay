		...
		function GC(memRatio) {
			...
			while (nodesArray.length > 0) { // iterate over all of our nodes
				...
				if (calcRemoveImportance(nodesArray[0][1], curTime, memRatio) >= 100) {
					// Check if root.gun exists before trying to access it
					if (root.gun && typeof root.gun.get === 'function') {
						root.gun.get(nodesArray[0][0]).off(); //Remove the node
					} else {
						// Fallback: remove from graph directly if gun instance is not available
						if (root.graph && root.graph[nodesArray[0][0]]) {
							delete root.graph[nodesArray[0][0]];
						}
					}
